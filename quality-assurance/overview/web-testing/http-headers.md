...
HTTP headers - это пары `name: value`, которые передают дополнительную информацию вместе с HTTP request или HTTP response.

Headers помогают client и server договориться о деталях:

- какой content type отправляется;
- какой language preferred;
- какие credentials используются;
- можно ли cache response;
- какие cookies установить;
- разрешен ли cross-origin request;
- какие security rules применяются.

Для QA headers важны потому, что многие bugs невозможно нормально понять только по UI. Нужно смотреть Network tab и видеть, какие headers ушли в request и какие вернулись в response.

Главная мысль:

> HTTP headers - это metadata разговора между client и server.

## What Is An HTTP Header?

HTTP header состоит из case-insensitive name, двоеточия и value.
...
Header name не чувствителен к регистру:
...
Для HTTP это одно и то же имя. Но в реальной документации и инструментах обычно используют привычный формат вроде `Content-Type`.

## Where QA Sees Headers

Headers можно увидеть:

- в browser DevTools на вкладке Network;
- в Postman, Insomnia или similar API tools;
- в curl output;
- в server logs;
- в proxy tools вроде Charles, Fiddler, mitmproxy;
- в automated API tests.

В DevTools обычно удобно смотреть отдельно:
...
Headers можно группировать по context.

| Type | Meaning |
| --- | --- |
| General headers | Относятся к message в целом, не только к body. |
| Request headers | Дают информацию о client, requested resource или request conditions. |
| Response headers | Дают информацию о response или server. |
| Entity / representation headers | Описывают body: type, length, encoding, language. |

В современной документации термины могут немного отличаться, но для QA важна практическая идея: часть headers уходит от client, часть приходит от server, часть описывает content.

## Request Headers

Request headers отправляются client to server.
...
Response headers возвращаются server to client.
