...
`PUT` и `PATCH` - это HTTP methods, которые часто используются для update resources через API.

Главное различие:

- `PUT` обычно заменяет resource целиком;
- `PATCH` изменяет только конкретные поля или части resource.

Для QA это важно, потому что неправильное использование `PUT` или `PATCH` может привести к потере данных, unexpected defaults, wrong validation или silent overwrites.

Главная мысль:

> `PUT` говорит "замени ресурс этим состоянием", а `PATCH` говорит "измени вот эти части ресурса".

## Why This Matters For QA

В UI это может выглядеть одинаково:

```text
User edited profile and clicked Save.
```

Но под капотом frontend может отправить:
...
Если frontend отправляет `PUT` с неполным body, server может стереть поля, которые не были переданы.

Если frontend отправляет `PATCH`, server должен изменить только переданные fields и оставить остальные без изменений.
...
Если API treats `PUT` as full replacement, missing fields can be lost.
...
Это зависит от API implementation, но QA должен явно проверять behavior.
