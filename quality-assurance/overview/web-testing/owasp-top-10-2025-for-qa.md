...
Language: Russian  
Translation pair: quality-assurance-en/overview/web-testing/owasp-top-10-2025-for-qa.md

## Summary

OWASP Top 10 - это awareness document о наиболее важных risks для web applications. Версия 2025 показывает, какие классы проблем остаются критичными для современных приложений: access control, misconfiguration, supply chain, cryptography, injection, design, authentication, integrity, logging and exceptional conditions.

Для QA это не checklist "проверить всё за один день", а карта рисков. Она помогает понимать, где web application чаще всего ломается с точки зрения безопасности, какие вопросы задавать команде и какие проверки учитывать при планировании testing.

Главная мысль:

> OWASP Top 10 помогает QA думать не только о functional bugs, но и о том, как приложение может быть использовано неправильно или небезопасно.

## Key Points

- OWASP Top 10:2025 сохраняет Broken Access Control как риск A01.
- Security Misconfiguration поднялась выше, потому что всё больше поведения приложения зависит от configuration.
- Software Supply Chain Failures стала отдельной широкой категорией вместо узкого фокуса только на outdated components.
- Injection и Cryptographic Failures остаются важными, но в 2025 они находятся ниже, чем в 2021.
- Новая категория A10:2025 - Mishandling of Exceptional Conditions.
- QA должен использовать OWASP как guide для risk-based testing, security questions, bug reports и collaboration with developers/security engineers.
...
Software Supply Chain Failures cover risks in dependencies, build systems, packages, containers and delivery pipeline.
