...
Language: Russian
Translation pair: articles-en/2026-05/week-04/04-how-switches-do-what-they-do.md

## Summary

Switch становится "умным", когда learns source MAC addresses из входящих frames и сохраняет связь MAC address -> port в CAM/MAC address table. Когда destination MAC известен, switch forwards frame только в нужный port. Когда destination MAC неизвестен или frame является broadcast, switch floods it out all ports except the incoming port.

Главная мысль: switching начинается с трех идей - source MAC learning, CAM table forwarding и ARP, который связывает IP address с MAC address внутри local network.
