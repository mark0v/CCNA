...
Language: Russian
Translation pair: articles-en/2026-05/week-04/02-understanding-mac-addresses-and-frames.md

## Summary

Чтобы понять switching, нужно начать с языка switch: MAC addresses and frames. Switch работает на Layer 2, Data Link layer, и принимает решения в первую очередь по MAC addresses, а не по IP addresses. MAC address - это local hardware identity сетевого интерфейса, а frame - Layer 2 wrapper, который несет source MAC, destination MAC и проверочную информацию вроде FCS.

Главная мысль: если ты понимаешь MAC addresses и Ethernet frames, дальнейшие темы про MAC address table, forwarding and device tracing становятся намного понятнее.
