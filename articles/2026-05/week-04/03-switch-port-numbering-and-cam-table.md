...
Language: Russian
Translation pair: articles-en/2026-05/week-04/03-switch-port-numbering-and-cam-table.md

## Summary

Switch port numbering и CAM table - это практические инструменты для понимания, что физически происходит в LAN. Interface names показывают тип и скорость port, numbering помогает ориентироваться в modules, stacks and ports, а CAM/MAC address table показывает, какие MAC addresses switch learned on which interfaces.

Главная мысль: switch output - это карта физической сети. Если ты умеешь читать port names, status and MAC address table, ты можешь быстрее найти device, определить uplink и понять, где начинается проблема.
