...
Language: Russian
Translation pair: articles-en/2026-05/week-04/07-what-now-switching-foundations.md

## Summary

После первого блока switching нужно понимать не просто команды, а смысл: switch соединяет устройства внутри local network, learns MAC addresses, builds a forwarding table и помогает находить devices по физическому подключению. Это фундаментальный troubleshooting skill, который превращает switch из "черного ящика" в источник evidence.

Главная мысль: switching сейчас ставится на паузу, но не заканчивается. Дальше начинается IP addressing, а позже VLANs, trunking and advanced switching станут намного понятнее на этом фундаменте.
