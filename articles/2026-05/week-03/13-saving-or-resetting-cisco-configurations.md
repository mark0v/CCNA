...
Language: Russian
Translation pair: articles-en/2026-05/week-03/13-saving-or-resetting-cisco-configurations.md

## Summary

Чтобы правильно сохранить или сбросить Cisco device, нужно понимать разницу между running configuration и startup configuration. Running config живет в RAM и показывает, что устройство делает прямо сейчас. Startup config хранится в NVRAM/flash and loads during boot. Если стереть только startup config, текущая running config продолжит работать до reload.

Главная мысль: factory reset обычно требует двух шагов - erase saved startup config, then reload without saving the current running config.
