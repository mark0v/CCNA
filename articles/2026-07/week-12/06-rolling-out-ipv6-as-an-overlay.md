...
Language: Russian
Translation pair: articles-en/2026-07/week-12/06-rolling-out-ipv6-as-an-overlay.md

## Summary

- IPv6 rollout часто начинается не с замены IPv4, а с overlay рядом с existing IPv4 network.
- В IPv6 subnetting обычно больше про organization, чем про conservation.
- Common design: ISP gives `/48`, site creates many `/64` LAN subnets.
- Fourth hextet is a clean place to encode internal subnet or VLAN structure.
- Cisco router needs `ipv6 unicast-routing`, otherwise it can have IPv6 addresses but not route IPv6 traffic.
