...
Language: Russian
Translation pair: articles-en/2026-07/week-12/04-ipv6-the-someday-protocol.md

## Summary

- IPv6 долго называли "someday protocol", потому что enterprise adoption шла медленнее, чем ожидали.
- Someday уже пришел, но в основном через carriers, ISPs, mobile networks и large internet services.
- Главная причина IPv6 - public IPv4 address exhaustion.
- IPv6 использует 128-bit address space, поэтому address scarcity перестает быть главным design pressure.
- В IPv6 common subnet size is /64: first 64 bits for network, last 64 bits for host.
- Для многих businesses full internal migration пока не имеет strong business case, но IPv6 нужно уметь читать и troubleshoot.
...
IPv6 не провалился. Он просто пришел не так, как многие ожидали.

Ожидание было таким: businesses однажды массово перестроят internal networks, заменят IPv4, и internet резко станет IPv6-first. Реальность другая. Большой push пришел от providers, которым public IPv4 address scarcity реально мешает расти.

Именно поэтому IPv6 может одновременно казаться rare внутри enterprise и normal на public internet.

## Why IPv4 Hit The Wall

IPv4 uses a 32-bit address space. В теории это примерно 4.2 billion addresses.

Проблема в том, что не все эти addresses usable as public internet addresses:
...
То есть проблема не просто математическая. Мы не можем freely hand out fresh public IPv4 space the way the early internet could.

Для обычной company это не всегда немедленная боль, потому что private addressing and NAT still work. NetworkChuck Coffee может использовать private IPv4 internally and hide many devices behind a smaller public address footprint.

Для carriers ситуация другая. Если provider обслуживает millions of customers and devices, public IPv4 scarcity становится прямой operational problem.

## Why Carriers Moved First

Carriers, ISPs and mobile providers сильнее всего чувствуют address pressure.
...
IPv6 дает providers больше room to grow. Поэтому "someday" для них стал "right now" раньше, чем для многих internal enterprise networks.

## The Address Space Change

IPv6 uses 128 bits instead of IPv4's 32 bits.

Это не маленькое увеличение. Это shift from "we must conserve addresses carefully" to "we can design with breathing room."
...
- support IPv6 where ISP, cloud or applications require it;
