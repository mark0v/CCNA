...
- can have its own data store;
- communicates with other services through lightweight protocols, often HTTP/REST or messaging.

For QA, microservices matter because a bug may not live in one application. It may live on the boundary between services:
...
> In microservices, QA tests not only a single feature, but also the chain of interactions between services.

## What Are Microservices?

Microservices is an approach where a large application is split into small services.

Each service solves its own task and can evolve separately.
...
Instead of one large application package, the system consists of smaller units.

## Monolithic Architecture

Monolithic architecture is an approach where all main parts of an application are combined into one deployable unit.

In a monolith, these parts often live together:
...
When a developer deploys a monolith, deployment happens as one package.

To scale the application, teams often run additional instances of the whole application, even if only Search has higher load.

## Microservice Architecture

Microservice architecture divides the system into autonomous services.
...
In the eCommerce example, Search can scale separately from Payments because users usually search for products more often than they pay for orders.
...
If the final result is wrong, QA needs to inspect the chain, not only the screen.

## Stateless And Stateful Services

## Stateless Service

A stateless service does not store request-specific state between requests.
...
An API service receives a token, validates the request, and returns a response without storing session state locally.

## Stateful Service

A stateful service keeps some state between requests.
...
In a monolith, a function call is local.
...
If a dependency is failing, stop calling it for a while and return fallback faster.
...
A monolith is deployed as one large unit. Microservices are split into independently deployable services.
