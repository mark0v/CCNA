...
It provides the user interface, displays information, collects user input, and sends user actions to the backend.

### 3. What does the application tier do?

It handles business logic, API behavior, validation, permissions, calculations, and communication with databases or external services.

### 4. Why should the presentation tier not talk directly to the data tier?

Because direct access weakens security, maintainability, and control over business rules. Data access should go through the application tier.

### 5. What is the difference between layer and tier?

A layer is a logical code separation. A tier is a physical or infrastructure-level runtime separation.

### 6. How can QA use three-tier thinking during bug investigation?

QA can trace the flow through UI, API, backend logic, database, response, and UI rendering to locate the level where the defect appears.
