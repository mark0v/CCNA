...
Localization testing and internationalization testing check whether a product can work for users from different languages, regions, and cultures.

In short:

- internationalization (`i18n`) means the product is technically ready for translation and multiple locales;
- localization (`l10n`) means the product is adapted for a specific language/region;
- globalization is the broader process of preparing a product for global markets.

Main idea:

> Internationalization makes the product ready for localization. Localization verifies that a specific local version looks and works naturally for local users.

## Why This Matters

For global web products, translating text is not enough.

Users expect the application to correctly show:
...
If localization is poor, users may not trust the product, may misunderstand actions, or may be unable to complete a flow.

## What Is Internationalization

Internationalization (`i18n`) is a design and development approach where an application is prepared in advance to support different languages, scripts, formats, and locales.

It is not the translation itself. It is technical readiness for translation.

Examples of i18n requirements:

- all user-facing text is moved from source code into resource files;
- application supports Unicode;
- UI can expand for longer text;
- layout supports right-to-left languages;
- date/time/number/currency formats depend on locale;
- database stores Unicode data;
- sorting and search follow language rules;
- code does not assume "one character = one byte";
- automated tests are not tied to English text.

## What Is Localization

Localization (`l10n`) is adapting a product, application, or content to a specific locale.

A locale usually includes:
...
If a database column or migration uses wrong encoding, `Марія`, `José`, `李雷` can become corrupted or unreadable.
