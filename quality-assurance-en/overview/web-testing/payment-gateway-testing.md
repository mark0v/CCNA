...
Payment gateway testing checks the integration between an e-commerce application and the payment provider through which a user pays for an order.

For QA, this is a high-risk area because defects can cause:
...
> Payment testing should verify not only the `Pay` button, but the whole transaction lifecycle: authorization -> capture -> confirmation -> settlement -> void/refund -> reconciliation.

## What Is A Payment Gateway

A payment gateway is a service that helps a merchant accept payment from a customer.

It usually:

- receives payment details;
- sends data to a payment processor/acquiring bank;
- receives approve/decline response;
- returns the result to the merchant application;
- supports refunds, voids, webhooks, fraud checks, and reporting.

In real projects, the terms `payment gateway` and `payment processor` are sometimes used interchangeably. For QA, it is more important to understand integration flow, callbacks/webhooks, statuses, and business rules.
