...
def clean_generated_docs() -> None:
    generated_articles = DOCS / "articles"
    if generated_articles.exists():
        shutil.rmtree(generated_articles)

    generated_plan = DOCS / "study-plan.md"
    if generated_plan.exists():
        generated_plan.unlink()


def copy_study_plan() -> None:
    shutil.copy2(ROOT / "STUDY_PLAN.md", DOCS / "study-plan.md")


def copy_articles() -> None:
    target_root = DOCS / "articles"
