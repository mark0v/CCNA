...
Language: English
Translation pair: articles/2026-05/week-04/09-why-do-we-need-ip-addresses.md

## Summary

MAC addresses are useful for local delivery inside one network segment, but they do not scale to a world made of many networks. If devices tried to find each other only through MAC-based broadcasts, the network would drown in noise. IP addresses provide hierarchy, structure and network boundaries, while routers connect those networks and keep broadcasts from spreading everywhere.

Main idea: MAC addresses deliver the frame to the next local hop, while IP addresses describe the final destination across networks.
