...
Language: English
Translation pair: articles/2026-05/week-04/05-exploring-mac-address-tables-on-a-cisco-switch.md

## Summary

The MAC address table becomes easier to understand when you see a switch learn addresses in real time. In Packet Tracer, you can build a simple topology, configure PC1 and PC2 in the same subnet, clear the ARP cache, run a ping and observe how the ARP request creates a broadcast, the switch learns the source MAC, and `show mac address-table` displays learned MAC addresses and ports.

Main idea: a switch learns first, forwards second. The MAC address table is a living map of the network that changes as traffic appears.
