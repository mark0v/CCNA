...
Language: English
Translation pair: articles/2026-05/week-05/13-connected-routes.md

## Summary

A connected route appears automatically when a router interface receives an IP address, is enabled and is operational. This is the first routing knowledge a router gets without static routes and without dynamic routing protocols.

Main idea: a router knows only the connected networks it is actually attached to through active interfaces. If an interface is down, the connected route disappears from the routing table. If a remote network sits behind another router, that network must be added through another routing method.
