...
Language: English
Translation pair: articles/2026-05/week-05/07-configuring-router-interfaces.md

## Summary

Router interface configuration is similar to switch interface configuration, but routers have important differences. The main one: router interfaces are often shutdown by default, so an IP address alone does not activate the interface. After configuration, you need `no shutdown`.

Main idea: a router interface must have the correct IP address, subnet mask, and be administratively enabled, or it will not work.
...
If you already worked with Cisco switch CLI, router CLI does not feel completely new.
...
But routers behave differently in important ways.
...
On a switch port, you may often see `down/down` when no cable is connected.
...
On a router interface, you may often see:
...
In other words, the router interface is turned off by configuration.
...
If you forget `no shutdown`, you can spend too long troubleshooting a "broken" link when the interface is simply disabled.
...
When a router connects to a switch in Packet Tracer, the switch link may show orange before turning green.
