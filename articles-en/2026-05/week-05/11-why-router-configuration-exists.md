...
Language: English
Translation pair: articles/2026-05/week-05/11-why-router-configuration-exists.md

## Summary

This lesson explains why router configuration exists at all. A switch is excellent inside a local network: it forwards frames between devices in the same LAN. But as soon as traffic needs to leave the local network, for example to the internet or another site network, a switch is not enough.

Main idea: switches make the local network useful, routers make the local network connected. The router becomes the device that connects different networks and gives local devices a path outward.
