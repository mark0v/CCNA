...
Language: English
Translation pair: articles/2026-05/week-05/09-interface-speed-and-transfer-time.md

## Summary

Interface speed should not be understood as an abstract "faster is better" number, but as practical capacity: how much data can be transferred over a period of time. To reason about that, you must not confuse bytes and bits, you need to convert file size into network speed units, and you need to account for real-world overhead.

Main idea: if you can estimate how long a transfer will take, you can design links, uplinks and network capacity more intelligently.

## Key Points

- Bytes usually measure file size and storage.
- Bits usually measure network transfer speed.
- `1 byte = 8 bits`.
- Uppercase `B` usually means bytes.
- Lowercase `b` usually means bits.
...
If the network is too slow, the business feels it.
