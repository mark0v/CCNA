...
Language: English
Translation pair: articles/2026-07/week-11/10-choosing-between-hsrp-vrrp-and-glbp.md

## Summary

- HSRP, VRRP, and GLBP solve the same family problem: resilient default gateway for hosts.
- HSRP and VRRP are very similar: one router forwards, another waits for failover.
- VRRP is standards-based; HSRP is Cisco proprietary.
- GLBP adds gateway load balancing, not just active/standby redundancy.
- In the real world, VRRP is often the practical choice; in Cisco/CCNA context, HSRP matters a lot.
...
FHRP protocols should not be treated as three completely unrelated technologies. They solve one basic problem: hosts need a default gateway, and that gateway should not depend on one physical router.

Instead of pointing clients to one router IP, FHRP lets routers share a virtual gateway identity. Hosts keep one default gateway IP. Behind the scenes, routers coordinate who owns that gateway right now.
...
Both provide a virtual IP for hosts. Both use a virtual MAC. Both allow failover if the primary gateway device fails.
...
Preemption means the preferred router can take back the active/master role when it comes back online.
...
One useful VRRP detail: the virtual gateway IP can be the real IP of one router. This can avoid using an extra address only for the virtual gateway.
...
You do not need to become a hex robot, but if you see an FHRP virtual MAC in ARP or a switch MAC table, recognition helps fast.
...
- clients can receive different virtual MAC addresses for the same gateway IP;
...
- VRRP is standards-based and common in the broader real world.
...
### 1. How are HSRP and VRRP similar?

Answer: Both provide a virtual default gateway and active/standby-style failover for hosts.

### 2. What is the main difference between HSRP and VRRP?

Answer: HSRP is Cisco proprietary, while VRRP is standards-based.

### 3. What is preemption?

Answer: It is the behavior where the preferred router takes the active/master role back after recovery.

### 4. How is GLBP different from HSRP/VRRP?

Answer: GLBP can load balance by giving different clients different virtual MAC addresses for the same gateway IP.

### 5. Why plan FHRP per VLAN?

Answer: Each VLAN/subnet has its own default gateway, so each one needs its own gateway redundancy design.
