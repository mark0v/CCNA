...
Language: English
Translation pair: articles/2026-07/week-13/01-standard-acls-safe-configuration-and-placement.md

## Summary

- Standard ACLs are simple because they match only source IP address.
- That simplicity is dangerous: a standard ACL can easily block more traffic than intended.
- Before deleting or changing an ACL, first check where it is applied.
- An ACL with only deny statements blocks the remaining traffic through implicit deny.
- Named ACLs are easier to maintain because names explain intent.
